...
struct DescriptionScreen: View {
    ...
    var body: some View {
        VStack{
            ...
            HStack{
                Text("다메다네 밈이란?").AutoSizeBinggraeFont(weight: .bold, textColor: .black, fontForWhat: .Title).padding(.leading,20)
                Spacer()
            }
            Text("최근에 커뮤니티나 인터넷에 보면 \"다메다네~ 다메요 다메다노요~\" 라며 노래를 부르는 것에 어떤 대상을 허접하게 딥페이크하는 영상이 인기를 끌고 있다.").multilineTextAlignment(.leading).padding(.all,20).AutoSizeBinggraeFont(weight: .medium, textColor: .black, fontForWhat: .Body)
            if let urlToYouTube = URL(string:"https://www.youtube.com/watch?v=4oxGyV6_Wgs") {
                Link(destination:urlToYouTube){
                    HStack{
                        ...
                        Text("유튜브 바로가기!").AutoSizeBinggraeFont(weight: .medium, textColor: Color("lightBlue"), fontForWhat: .Body)
                        Spacer()
                    }
                }
            }
            HStack{
                Text("딥페이크란?").AutoSizeBinggraeFont(weight: .bold, textColor: .black, fontForWhat: .Title).padding(.leading,20).padding(.top,50)
                Spacer()
            }
